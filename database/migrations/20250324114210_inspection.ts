...
export async function up(knex: Knex): Promise<void> {
  return knex.schema.createTable('inspection', (table) => {
    ...
    table.string('purchase_plan_type').checkIn(['Mortgage', 'Outright Purchase', 'Installment']).notNullable();
    table.string('meeting_platform').checkIn(['WhatsApp', 'Google Meet', 'Zoom', 'Teams', 'FaceTime']);
