...
export class PrequalifyRepository implements IPreQualify {
  public async storePropertyInfo(
    input: propertyInformation,
  ): Promise<propertyInformation> {
    const [property] = await db('prequalify_property_information')
      .insert(input)
      .returning('*')
  ...
  public async storeEmploymentInfo(
    ...
    const [employment] = await db('prequalify_employment_information')
      .insert(input)
      .returning('*')
    return new employmentInformation(employment) ? employment : null
  }

  public async storeFinancialInfo(
    input: financialInformation,
  ): Promise<financialInformation> {
    const [financial] = await db('prequalify_financial_information')
      .insert(input)
      .returning('*')
