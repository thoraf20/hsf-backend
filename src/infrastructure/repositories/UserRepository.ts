...
export class UserRepository implements IUserRepository {
  ...
  async findById(id: string): Promise<any> {
      ...
      .where({id}).first()
      const role = await db('roles').select('roles.name as role').where({id: user.role_id}).first()
      return {...user, ...role}
  }
 ...
 public async getRoleByName(name: string): Promise<Record<string, any> | null> {
    return db('roles').where('name', name).first()
}

 public async comparedPassword(input: string, hashed: string): Promise<string | boolean> {
