...
enquiryRoutes.post(
  ...
  asyncMiddleware(async (req, res) => {
    const { user, body } = req
    const enquiry = await enquiryController.startEnquiryTrail(user.id, body)
...
enquiryRoutes.patch(
  '/property',
  requireRoles([Role.HOME_BUYER, Role.DEVELOPER_ADMIN]),
  validateRequest(continueEnquirySchema),
  asyncMiddleware(async (req, res) => {
    const { user, body } = req
    const enquiry = await enquiryController.continueEnquiryTrail(user.id, body)
    res.status(enquiry.statusCode).json(enquiry)
  }),
)
...
enquiryRoutes.post(
  ...
  asyncMiddleware(async (req, res) => {
    ...
    const enquiry = await enquiryController.closeEnquiry(user.id, params.id)
    res.status(enquiry.statusCode).json(enquiry)
  }),
)
...
enquiryRoutes.get(
  ...
  asyncMiddleware(async (req, res) => {
    const { user, params } = req
    const enquiry = await enquiryController.getEnquiryInfo(user.id, params.id)
    res.status(enquiry.statusCode).json(enquiry)
  }),
)
...
enquiryRoutes.get(
  ...
  asyncMiddleware(async (req, res) => {
    const { user } = req
    const enquiry = await enquiryController.getEnquiries(user.id)
    res.status(enquiry.statusCode).json(enquiry)
  }),
)
