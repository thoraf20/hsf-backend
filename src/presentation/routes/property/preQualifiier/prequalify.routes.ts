...
import { asyncMiddleware, requireRoles, Role, validateRequest } from '@presentation/routes/index.t'
...
preQualifierRoutes.post('/request', requireRoles(Role.HOME_BUYER), validateRequest(preQualifySchema), asyncMiddleware(async(req, res) => {
            const {user, body} = req 
            const store = await controller.preQualifierController(body, user.id)
            res.status(store.statusCode).json(store)
}))

preQualifierRoutes.post('/verification', requireRoles(Role.HOME_BUYER), validateRequest(verifyOtpSchema),  asyncMiddleware(async (req, res) => {
            const {body} = req 
