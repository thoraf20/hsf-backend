...
import { LoanOffer, uploadDocument, uploadPrecedentDocument } from '@entities/PurchasePayment'
...
export interface IMortageRespository {
  ...
  uploadPrecedentDocument(input: uploadPrecedentDocument): Promise<uploadPrecedentDocument>
  savePaymentStatus(
    property_id: string,
    user_id: string,
  ): Promise<MortagePayment>
  getPaymentStatusByIds(property_id: string, user_id: string): Promise<MortagePayment>
  updateLoanOffer (input: LoanOffer, property_id: string, user_id: string): Promise<void> 
