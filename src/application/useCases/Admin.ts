...
import { generateRandomSixNumbers, generateDefaultPassword } from "../../utils/helpers";
   ...
   public async inviteAgents(input: User): Promise<User> {
          await this.existingUsers.beforeCreate(input.email, input.phone_number)
           ...
           if(!checkRole)
            ...
            const password = await this.userRepository.hashedPassword(generateDefaultPassword())
