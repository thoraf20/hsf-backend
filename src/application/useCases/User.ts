    ...
    private readonly client = new RedisClient()
      ...
      public async update(input: User, id: string): Promise<void> {
           ...
           const [existedEmail, existedPhone] = await Promise.all([
                input.email ? this.userRepository.findByPhone(input.email) : Promise.resolve(null),
                input.phone_number ? this.userRepository.findByPhone(input.phone_number) : Promise.resolve(null)
           ])


           if(existedEmail && existedEmail.id !== id) {
               throw new ApplicationCustomError(StatusCodes.CONFLICT, 'Email already exists')
           }

           if(existedPhone && existedPhone.id !== id) {
               throw new ApplicationCustomError(StatusCodes.CONFLICT, 'Phone number already exists')
           }
           ...
           if(input.email) {
               ...
               const details = { id, token, type: OtpEnum.EMAIL_UPADTE, newEmail: input.email }
               await this.client.setKey(key, details, 60)
               throw new ApplicationCustomError(StatusCodes.BAD_REQUEST, 'A verification email has been sent. Please verify your new email before updating.')
           }
      ...
      public async verifyUpdate (otp: string): Promise<void> {
        ...
        if (!data) throw new ApplicationCustomError(StatusCodes.BAD_REQUEST, "Invalid or expired otp");
        console.log(data)
      ...
      public async resetPassword(input: resetPassword,id: string): Promise<void> {
              ...
              const isOldPasswordValid = await this.userRepository.comparedPassword(input.oldPassword, user.password);
              if (!isOldPasswordValid) {
                  throw new ApplicationCustomError(StatusCodes.UNAUTHORIZED, "Old password is incorrect");
              }
              
              if(input.oldPassword === input.newPassword) {
                  throw new ApplicationCustomError(StatusCodes.BAD_REQUEST, "New password can't be the same as old password");
              }
              
              const hashedNewPassword = await this.userRepository.hashedPassword(input.newPassword);
              await this.userRepository.update(id, { password: hashedNewPassword });

      }
      public async EnableAndDisableMfa(id: string): Promise<{ is_mfa_enabled: boolean }> {
        const user = await this.userRepository.findById(id);
        if (!user) {
            throw new ApplicationCustomError(StatusCodes.NOT_FOUND, "User not found");
        }
