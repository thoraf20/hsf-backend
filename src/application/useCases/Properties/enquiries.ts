...
export class EnquiryService {
    ...
    constructor(enquiryRepository: IEnquiresRepository, propertyRepository: IPropertyRepository ){
        this.enquiryRepository = enquiryRepository;
        this.propertyRepository = propertyRepository;
        this.utilsEnquiry = new EnquiryBaseUtils(this.enquiryRepository, this.propertyRepository);
    }
    ...
    public async makeAnEnquiry(
        ...
        const enquiryTrail = await this.enquiryRepository.newMessageEnquiry(enquiry, enquiryMsg)
        return enquiryTrail
    }
    ...
    public async respondToEnquiry(
        ...
        let enquiryTrail = await this.utilsEnquiry.confirmEnquiryExistForUser(enquiry_id, user_id);
        let enquiryMsg = new EnquiryMsg({
            owner_id: user_id,
            message: message,
        })
        enquiryTrail = await this.enquiryRepository.continueEnquiry(enquiryTrail.id, enquiryMsg)
        return enquiryTrail
    }

    public async closeEnquiry(
        enquiry_id: string,
        user_id: string,
    ) {
        let enquiryTrail = await this.utilsEnquiry.confirmEnquiryExistForUser(enquiry_id, user_id);
