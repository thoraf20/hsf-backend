...
export class ManageAgents {
  ...
  public async resetOrganizationAgentPassword(
    ...
    const getAgent = getUserClientView(await this.userRepository.findById(agent_id))
    if(!getAgent) {
      throw new ApplicationCustomError(
        StatusCodes.NOT_FOUND,
        'Agent not found',
    ...
    const hashedPassword = await this.userRepository.hashedPassword(defaultPassword)
     ...
     template.passwordResetForOrganization(getAgent.email, `${getAgent.first_name} ${getAgent.last_name}`, defaultPassword, url, getOrganization.name)
  }
