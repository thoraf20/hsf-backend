import { z } from 'zod';
import { MeetingPlatform, InspectionMeetingType, FinancialOptionsEnum } from '../../../domain/enums/propertyEnum';
...
export const inspectionSchema = z.object({
    purchase_plan_type: z.nativeEnum(FinancialOptionsEnum, {
      errorMap: () => ({ message: 'Invalid purchase plan type. Choose from Mortgage, Outright Purchase, or Installment.' }),
    }),
  
    inspection_date: z.string()
      .regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format')
      .refine(isFutureDate, { message: 'Inspection date must be in the future.' }),
    ...
    contact_number: z.string()
      .regex(/^\+?\d{7,15}$/, 'Phone number must be between 7-15 digits and may start with "+".'),
    ...
    .refine(value => parseFloat(value) > 0, { message: 'Amount must be a positive number.' }).optional(),
    meeting_link: z.string().url({ message: 'Invalid meeting link format.' }).optional(),
